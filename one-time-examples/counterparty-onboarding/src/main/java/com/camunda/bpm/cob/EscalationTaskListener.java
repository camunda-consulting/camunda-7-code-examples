...
public class EscalationTaskListener implements TaskListener {
	...
	private static final Logger log = Logger.getLogger(EscalationTaskListener.class.getName());  
	...
	private void createJobForEscalation(DelegateTask delegateTask) {
		...
		TimerDeclarationImpl timerDeclaration = 
				new TimerDeclarationImpl(
						...
						EscalationJobHandler.ESCALATION_JOB_HANDLER_TYPE);
	...
	private void deleteJobForEscalation(DelegateTask delegateTask) {
		...
		for (JobEntity jobEntity : jobs) {
			if (jobEntity.getJobHandlerType().equals(EscalationJobHandler.ESCALATION_JOB_HANDLER_TYPE)) {
				jobEntity.delete();
