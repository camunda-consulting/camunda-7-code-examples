...
public class CaptureRelevantHistoricItemsListener implements ExecutionListener {
	...
	public void notify(DelegateExecution execution) throws Exception {
		...
		// find DMN decisions
		HistoricDecisionInstance determineSkillsDecision = execution.getProcessEngineServices().getHistoryService().createHistoricDecisionInstanceQuery().processInstanceId(decisionFlowPI.getId()).decisionDefinitionKey(BpmConstants.DECISION_DEFINITION_KEY_determineSkills).includeOutputs().orderByEvaluationTime().desc().list().get(0);		
		...
		List<HistoricDecisionInstance> list = execution.getProcessEngineServices().getHistoryService().createHistoricDecisionInstanceQuery().processInstanceId(decisionFlowPI.getId()).decisionDefinitionKey(BpmConstants.DECISION_DEFINITION_KEY_scoreEmployee).includeOutputs().orderByEvaluationTime().desc().list();
