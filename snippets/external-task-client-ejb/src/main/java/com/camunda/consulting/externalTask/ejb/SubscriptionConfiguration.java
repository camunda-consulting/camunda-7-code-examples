...
import static com.camunda.consulting.externalTask.ejb.ExternalTaskSubscription.*;
...
public class SubscriptionConfiguration {
  ...
  protected static boolean isNull(ProcessVariable[] values) {
    return values.length == 1
        && STRING_NULL_VALUE.equals(values[0].name())
  ...
  public void fromAnnotation(ExternalTaskSubscription config) {
    ...
    setProcessDefinitionIdIn(
        isNull(processDefinitionIdIn) ? null : Arrays.asList(processDefinitionIdIn));
    ...
    setProcessDefinitionKeyIn(
        isNull(processDefinitionKeyIn) ? null : Arrays.asList(processDefinitionKeyIn));

    String processDefinitionVersionTag = config.processDefinitionVersionTag();
    setProcessDefinitionVersionTag(
        isNull(processDefinitionVersionTag) ? null : processDefinitionVersionTag);

    ProcessVariable[] processVariables = config.processVariables();
    setProcessVariables(
        isNull(processVariables)
            ? null
