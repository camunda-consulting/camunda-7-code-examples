...
Manual archiving of Camunda History tables:
   ...
   PARAMS:
     IN_periodInDays        : archive all ProcessInstances with END_TIME_ + IN_periodInDays <= CIN_periodInDays seit so vielen Tagen gesetzt ist UND AUFTRAGSKORBNUMMER abgeschlossen!
                              (DEFAULT=1)
	...
	HOW TO EXECUTE: 
		SET SERVEROUTPUT ON;
		DBMS_OUTPUT.enable;
	    select ARCHIVE_CAMUNDA_HISTORY(1, 10) from DUAL; -- Archive max. 10 PI's, older then 1 day
	    --DBMS_OUTPUT.disable;
